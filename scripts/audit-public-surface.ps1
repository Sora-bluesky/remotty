...
Assert-FileContains -Path 'docs/telegram-quickstart.md' -Needle 'define `$configPath` in that window first'
Assert-FileContains -Path 'docs/telegram-quickstart.md' -Needle 'This line only stores the config file path'
...
Assert-FileContains -Path 'docs/telegram-quickstart.ja.md' -Needle 'その PowerShell でも先に `$configPath` を設定'
Assert-FileContains -Path 'docs/telegram-quickstart.ja.md' -Needle '設定ファイルの場所を PowerShell 変数に入れるだけ'
