...
2. 同じプロジェクト、同じ Windows ユーザーで、別の PowerShell ウィンドウを使って `remotty remote-control` を起動します。
3. `%APPDATA%\remotty\bridge.toml` がない場合は、`remotty` が作成します。`remotty` は起動のたびに現在のプロジェクトを登録し、Telegram bot token が未保存の場合だけ入力を求めます。
...
- bot token を保存する Windows ユーザーと、`remotty` を起動する Windows ユーザーが同じであること
...
Telegram bot token が未保存の場合は、表示に従って token を貼ります。
...
起動時は、既定では `%APPDATA%\remotty\bridge.toml` の設定を使います。
必要なら `remotty` がこのファイルを作り、現在のプロジェクトを登録します。
...
> 下位レベルの `remotty config workspace upsert` コマンドも、高度なスクリプト向けに利用できます。
...
> `remotty` を起動する Windows ユーザーは、token を保存したユーザーと同じである必要があります。
