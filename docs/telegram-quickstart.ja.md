...
>
> A. ブリッジが実行中ターンを見つけられる場合、テキストはキューに入ります。
> キューに入ったテキストは、現在の処理が受け付け可能になってから渡されます。
> 実行中ターンがある間、添付はキューに入りません。
> 添付は現在のターンが終わってから送ってください。
> ブリッジ再起動後は、古いターンが既に終了していることがあります。
> その場合は復旧し、選択済みスレッドで新しい依頼を開始します。
